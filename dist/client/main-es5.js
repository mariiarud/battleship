...
var AppComponent = /** @class */ (function () {
    function AppComponent(notifier) {
        this.timeLeft = 2;
    ...
    AppComponent.prototype.ngAfterViewInit = function () {
        var _this = this;
        this.socket.on("callReset", function () {
            _this.isCall = false;
        });
        this.socket.on("roomStatus", function (data) {
            _this.rooms = data;
        });
        this.socket.on("newPlayer", function (id, room) {
            _this.playerId = id;
            _this.gameStatus = 'new';
            _this.currentRoom = room;
        });
        this.socket.on("roomIsFull", function () {
            _this.notifier.notify("warning", "Room is occupied!");
        });
        this.socket.on("rollCall", function (id) {
            if (_this.playerId == '' && !_this.isCall) {
                _this.isCall = true;
                _this.socket.emit("iAmGuest");
            }
            else if (_this.playerId == id)
                _this.socket.emit("iAmHere", _this.playerId, _this.currentRoom);
        });
        this.socket.on("startNewGame", function () {
            _this.gameStatus = 'start';
        });
        this.socket.on("playerLeave", function (room) {
            if (_this.currentRoom == room) {
                _this.notifier.notify("error", "Opponent left the game!");
    ...
    AppComponent.prototype.joinRoom = function (i) {
...
var NewGameComponent = /** @class */ (function () {
    function NewGameComponent(notifier) {
        this.isReady = false;
    ...
    NewGameComponent.prototype.ngAfterViewInit = function () {
        var _this = this;
        this.socket.on("waiting", function () {
            _this.isReady = true;
            _this.notifier.notify("info", "Waiting for the second player...");
            document.getElementById('eraser').style.cssText = "background-color: #ff181870;";
        });
    };
    NewGameComponent.prototype.takeShip = function (i) {
        this.shipToMove = 4 - i;
    ...
    NewGameComponent.prototype.putShip = function (i, j) {
        if (i != 0 && j != 0) {
            if (this.putToBorder(i - 1, j - 1)) {
                var id = void 0;
                if (this.isVerticalMode) {
                    for (var n = 0; n < this.shipToMove; n++) {
                        ///////////////
                        var k = n;
                        //////////////
                        if ((i + n) >= 11) {
    ...
    NewGameComponent.prototype.putToBorder = function (iBase, jBase) {
        var testBord = this.copyBoard(this.myBoard);
        var i = iBase;
        var j = jBase;
        if (this.isVerticalMode) {
            ...
            for (var y = i - 1; y <= i + this.shipToMove; y++) {
                if (y >= 0 && y < 10) {
            ...
            for (var x = j - 1; x <= j + this.shipToMove; x++) {
                if (x >= 0 && x < 10) {
    ...
    NewGameComponent.prototype.shipInAir = function (i, j) {
    ...
    NewGameComponent.prototype.flewAway = function () {
    ...
    NewGameComponent.prototype.copyBoard = function (arr) {
    ...
    NewGameComponent.prototype.turnOver = function () {
    ...
    NewGameComponent.prototype.eraser = function () {
        if (!this.isReady) {
            for (var i = 0; i < 10; i++) {
                for (var j = 0; j < 10; j++) {
                    var id = void 0;
    ...
    NewGameComponent.ctorParameters = function () { return [
...
var OnGameComponent = /** @class */ (function () {
    function OnGameComponent(notifier) {
        this.myBoard = new Array();
    ...
    OnGameComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.notifier.notify("success", "Start!");
        this.socket.on("myBoard", function (data) {
            _this.myBoard = data;
            _this.refreshBoard(_this.myBoard, "my");
        });
        this.socket.on("enemyBoard", function (data) {
            _this.realEnemyBoard = data;
        });
        this.socket.on("isTurn", function (data) {
            _this.isTurn = data;
            _this.changeTurn();
        });
        this.socket.emit("getBoards", this.playerId, this.currentRoom);
    };
    OnGameComponent.prototype.ngAfterViewInit = function () {
        var _this = this;
        this.socket.on("renewBoard", function (board) {
            _this.myBoard = board;
            _this.refreshBoard(_this.myBoard, "my");
        });
        this.socket.on("turn", function (room) {
            if (_this.currentRoom == room) {
                _this.isTurn = !_this.isTurn;
                _this.changeTurn();
            }
        });
        this.socket.on("win", function (id, score) {
            _this.gameStatus = 'win';
            _this.notifier.notify("success", "You win!");
        });
        this.socket.on("lose", function () {
            _this.gameStatus = 'lose';
            _this.notifier.notify("error", "You lose!");
            for (var i = 0; i < 10; i++) {
                for (var j = 0; j < 10; j++) {
                    if (_this.realEnemyBoard[i][j] == 1) {
                        _this.myEnemyBoard[i][j] = 1;
                        var id = "en" + (i + 1) + "_" + (j + 1);
    ...
    OnGameComponent.prototype.changeTurn = function () {
        if (this.isTurn)
            this.gameStatus = 'play';
        else
            this.gameStatus = 'wait';
    };
    OnGameComponent.prototype.refreshBoard = function (board, name) {
        //●;
        for (var i = 0; i < 10; i++) {
            for (var j = 0; j < 10; j++) {
                var id = name + (i + 1) + "_" + (j + 1);
                switch (board[i][j]) {
                    case (1):
                        document.getElementById(id).style.cssText = "background-color: #4b4b4b95;";
                        break;
    ...
    OnGameComponent.prototype.aimOn = function (i, j) {
        if (i > 0 && j > 0) {
    ...
    OnGameComponent.prototype.aimOut = function (i, j) {
        if (i > 0 && j > 0) {
            if (this.myEnemyBoard[i - 1][j - 1] == 0) {
                var id = "en" + (i) + "_" + (j);
                document.getElementById(id).style.cssText = "background-color: #0c74eb0;";
    ...
    OnGameComponent.prototype.move = function (iBase, jBase) {
        if (iBase > 0 && jBase > 0) {
            var i = iBase - 1;
            var j = jBase - 1;
    ...
    OnGameComponent.prototype.killChack = function (i, j) {
        // this.notifier.notify( "warning", "killChack" );
        var _this = this;
        var map = new Map();
        var chackBoard = new Array();
        ...
        if (!map.has(1)) {
            this.ship.forEach(function (e) {
                _this.realEnemyBoard[e[1]][e[0]] = 5;
                var id = "en" + (e[1] + 1) + "_" + (e[0] + 1);
                document.getElementById(id).style.cssText = "background-color: #1eff0095;";
            });
        }
        else {
    ...
    OnGameComponent.prototype.serch = function (map, i, j, arr) {
        arr[i][j] = 0;
        for (var x = j - 1; x <= j + 1; x++) {
            if (x >= 0 && x < 10)
                for (var y = i - 1; y <= i + 1; y++) {
    ...
    OnGameComponent.prototype.copyBoard = function (arr) {
        var newArrey = new Array();
        for (var i = 0; i < arr.length; i++) {
            newArrey[i] = [];
            for (var j = 0; j < arr[i].length; j++) {
    ...
    OnGameComponent.ctorParameters = function () { return [
        { type: angular_notifier__WEBPACK_IMPORTED_MODULE_2__["NotifierService"] }
    ]; };
    tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
        Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])(),
